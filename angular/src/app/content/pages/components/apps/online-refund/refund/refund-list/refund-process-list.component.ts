...
@Component({
    selector: 'm-refund-process-list',
...
export class RefundProcessListComponent implements OnInit {
    // Table fields
    dataSource: RefundDataSource;
    displayedColumns = ['processedDate','refundNumber', 'fullName', 'refundAmount', 'status', 'reason', 'aging', 'actions'];
    ...
    loadProcessRefundList() {
        ...
        this.dataSource.loadProcessRefund(queryParams);
    }
    ...
    getComponentTitle() {
        let result = 'Processing Refund List';
        return result;
    }
}
